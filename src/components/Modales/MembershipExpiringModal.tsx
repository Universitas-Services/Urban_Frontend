            ...
            <DialogContent className="sm:max-w-[440px] p-6 gap-0 rounded-3xl" showCloseButton={false}>
                <div className="flex flex-col items-center">
                    ...
                    <DialogHeader className="p-0 mb-3">
                        <DialogTitle className="font-display text-3xl font-bold text-primary text-center leading-tight">
                            {daysLeft <= 0 ? 'Membresía vencida' : 'Membresía por vencer'}
                        </DialogTitle>
                    </DialogHeader>

                    <p className="text-gray-dark text-center text-[15px] leading-relaxed mb-4 px-4">
                        {daysLeft <= 0
                    ...
                    <div className="w-full h-2 bg-surface-soft rounded-full mb-6 overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all duration-500 ${daysLeft <= 0 ? 'bg-red-500' : 'bg-accent'}`}
                            style={{ width: daysLeft <= 0 ? '100%' : '85%' }}
                        />
                    </div>

                    <div className="bg-surface-light p-4 rounded-xl w-full mb-6">
                        <h4 className="text-primary font-bold text-center text-sm mb-4">
                            Actualiza tu plan ahora para continuar sin interrupciones.
                        </h4>
                        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2">
                            <div className="flex items-center gap-2">
                                <CheckCircle2 className="w-5 h-5 text-accent" />
                                <span className="text-gray-dark text-sm font-medium">Acceso completo</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <CheckCircle2 className="w-5 h-5 text-accent" />
                                <span className="text-gray-dark text-sm font-medium">Historial completo de chats</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col w-full gap-3">
                        <Button
                            ...
                            className="bg-accent hover:bg-accent-hover text-white rounded-xl h-12 text-base font-semibold shadow-sm transition-all"
                        >
                            Renovar membresía
                        </Button>
                        <Button
                            variant="ghost"
                            onClick={onClose}
                            className="text-gray-dark hover:text-primary hover:bg-transparent text-base font-medium transition-all h-10"
                        >
