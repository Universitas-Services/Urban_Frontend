...
import type { InicioActualidadDestacado } from './inicio.data';

function encodePublicPdfPath(path: string): string {
    const segments = path.split('/').filter(Boolean);
    return `/${segments.map((segment) => encodeURIComponent(segment)).join('/')}`;
}

export function InicioActualidad() {
    const [selectedItem, setSelectedItem] = useState<InicioActualidadDestacado | null>(null);

    const selectedPdfUrl = useMemo(
        () => (selectedItem ? encodePublicPdfPath(selectedItem.pdfPath) : null),
        [selectedItem]
    );
                    ...
                    {INICIO_ACTUALIDAD_DESTACADOS.map((item) => (
                        <li key={item.id}>
                            <button
                                ...
                                className="w-full rounded-lg border border-gray-100 bg-surface-soft/30 px-3 py-2.5 text-left transition-colors hover:border-primary/15 hover:bg-surface-soft/60"
                            >
