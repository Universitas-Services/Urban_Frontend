...
import { getAdminChatMessagesAction } from '@/lib/services/admin-chat.service';
import { ChatConversation, ChatMessage } from '@/types/admin-chat.types';
...
export function ChatWindow({ chat, onBack, className }: ChatWindowProps) {
    ...
    useEffect(() => {
        if (chat) {
            getAdminChatMessagesAction(chat.id).then((data) => {
                setMessages(data);
                setIsLoading(false);
            });
        }
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/10">
                {isLoading ? (
                    <div className="flex h-full items-center justify-center">
                        <span className="text-sm text-muted-foreground animate-pulse">Cargando historial...</span>
                    </div>
                ) : (
                    messages.map((msg) => (
                        <ChatMessageBubble key={msg.id} message={msg} userAvatar={chat.user.avatar} />
                    ))
                )}
