...
export default function DashboardPage() {
    ...
    const fetchMetrics = async (options?: { silent?: boolean }) => {
        try {
    ...
    const statCards = [
        {
            ...
            badge: (() => {
                ...
                const growth = calculateGrowth(
                    metrics.analytics.comparativa.mensual.actual,
                    metrics.analytics.comparativa.mensual.anterior
                );
        ...
        },
        {
            ...
            icon: Building2,
            color: ADMIN_KPI_COLORS.territorial.color,
            bgColor: ADMIN_KPI_COLORS.territorial.bgColor,
            badge: 'Públicos',
        ...
        },
        {
            ...
            icon: ShieldCheck,
            color: ADMIN_KPI_COLORS.success.color,
            bgColor: ADMIN_KPI_COLORS.success.bgColor,
            badge: 'Verificados',
            link: '/admin/usuarios?estadoCuenta=ACTIVO',
        },
        {
        ...
        },
        {
            ...
            icon: Ban,
            color: ADMIN_KPI_COLORS.danger.color,
            bgColor: ADMIN_KPI_COLORS.danger.bgColor,
            badge: 'Recientes',
            link: '/admin/usuarios?estadoCuenta=SUSPENDIDO',
        },
    ];

    // Datos para el gráfico de barras (Invertido)
    const chartData =
        metrics?.analytics.graficoCrecimiento
            .slice()
            .reverse()
            .map((item, idx, arr) => {
    ...
    const donutData = [
        {
            name: 'Públicos',
            value: metrics?.analytics.porTipousuario.servidoresPublicos || 0,
            color: ADMIN_CHART_COLORS.publicos,
        },
        {
            name: 'Asesores',
                    ...
                    <CardHeader className="flex flex-row items-center justify-between pb-6">
                        ...
                        <Link href="/admin/usuarios">
                            <button
                                ...
                                style={{ color: 'var(--admin-text-link)' }}
                            >
                                Ver todos
                            </button>
                        </Link>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="border-b border-gray-100 hover:bg-transparent">
                                    <TableHead className="text-[11px] font-bold tracking-wider text-muted-foreground uppercase h-10 w-[350px]">
                                        Nombre
                                    </TableHead>
                                    <TableHead className="text-[11px] font-bold tracking-wider text-muted-foreground uppercase h-10 w-[250px] text-center">
                                        Tipo
                                    </TableHead>
                                ...
                                {metrics?.recentUsers.map((user) => (
                                    ...
                                    >
                                        ...
                                        <TableCell className="py-4 text-center">
                                            ...
                                            >
                                                {getAccountStatusStyle(user.estadoCuenta).label ||
                                                    user.estadoCuenta ||
                                                    'Desconocido'}
                                            </Badge>
                                        </TableCell>
