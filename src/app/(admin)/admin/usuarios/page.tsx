...
export default function UsuariosPage() {
    ...
    const ejecutarEliminacionMasiva = async () => {
        ...
        try {
            await bulkDeleteUsersAction(selectedUserIds);
    ...
    const fetchUsers = useCallback(async (currentFilters: GetUsersParams, signal?: AbortSignal) => {
        setLoading(true);
        try {
            const response = await getAllUsersAction(currentFilters);
    ...
    const fetchEstados = useCallback(async () => {
        try {
            const response = await getEstadosAction();
            setEstados(response);
        } catch (error) {
    ...
    const fetchMunicipios = useCallback(async (estadoId: number) => {
        ...
        try {
            const response = await getMunicipiosAction(estadoId);
            setMunicipios(response as Municipio[]);
        } catch (error) {
            ...
            <div className="mt-4 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col w-full">
                                ...
                                >
                                    <SelectTrigger className="bg-white border-none rounded-lg h-11 font-medium text-slate-800">
                                        <SelectValue
                                            placeholder={loadingTerritorio ? 'Cargando...' : 'Seleccionar Municipio'}
                                        />
                                    </SelectTrigger>
                                    <SelectContent>
                    ...
                    >
                        ...
                        <div className="flex items-center gap-6 mt-4 md:mt-0">
                            ...
                            <Button
                                onClick={applySearch}
                                className="bg-primary hover:bg-primary-hover text-on-primary px-8 py-5 h-11 rounded-lg font-bold text-sm"
                            >
                                Aplicar Filtros
                            </Button>
                        </div>
                                    ...
                                    users.map((user) => {
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell className="py-5">
                                                    {(() => {
                                                        const config = getAccountStatusStyle(user.estadoCuenta);
                                                        return (
                                                            <Badge
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-6">
                            <span className="text-xs font-semibold text-muted-foreground">
                                ...
                                <span className="text-slate-900">
                                    ...
                                    {meta ? Math.min(meta.currentPage * meta.itemsPerPage, meta.totalItems) : 0}
                                </span>{' '}
                                de <span className="text-slate-900">{meta?.totalItems || 0}</span> usuarios registrados
                            </span>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-6 pb-12">
                    {statCards.map((stat, idx) => {
                        ...
                        const isActive =
                            filter &&
                            ((filter.type === 'tipo' && filters.tipoUsuario === filter.value) ||
