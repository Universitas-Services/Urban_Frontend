...
export default function AbandonedRegistrationsPage() {
    ...
    const ejecutarEliminar = async (id: string) => {
        try {
            await deleteAbandonedRegistrationAction(id);
    ...
    const fetchUsers = useCallback(async (currentFilters: typeof filters, signal?: AbortSignal) => {
        setLoading(true);
        try {
            const response = await getAbandonedRegistrationsAction(currentFilters);
                                ...
                                <TableRow>
                                    <TableCell colSpan={5} className="h-[300px] text-center">
                                        <div className="flex flex-col items-center justify-center text-muted-foreground">
                                            ...
                                            <p className="text-sm font-semibold tracking-wide">Cargando registros...</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : users.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="h-[300px] text-center">
                                        <div className="flex flex-col items-center justify-center text-muted-foreground">
                                            ...
                                            <h3 className="text-lg font-bold text-slate-900 mb-1">No hay registros</h3>
                                            <p className="text-sm">
                                                No se encontraron usuarios abandonados con los filtros actuales.
                                            </p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-5 border-t border-gray-200/60 bg-[var(--admin-filter-bg)]">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        <span className="text-xs font-semibold text-muted-foreground">
                            ...
                            de <span className="text-slate-900">{meta?.totalItems || 0}</span> registros
                        </span>
