...
export default function AbandonedRegistrationDetailsPage() {
    ...
    const fetchUser = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const userData = await getAbandonedRegistrationByIdAction(id);
            setUser(userData);
        } catch (error) {
    ...
    const handleAddNote = async () => {
        ...
        try {
            await addAbandonedRegistrationNoteAction(id, {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                ...
                <p className="text-slate-500 font-medium animate-pulse">Cargando detalles del registro...</p>
            </div>
        );
    }
    ...
    if (!user) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                <p className="text-slate-500 font-medium">No se encontró el registro abandonado.</p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-2xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-colors">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <p className="font-bold text-slate-800 text-sm leading-snug">
                                            {format(new Date(user.registeredAt), 'dd MMM yyyy HH:mm', { locale: es })}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col mt-4 bg-slate-50/50 rounded-2xl border border-slate-100 p-2 overflow-hidden flex-1 h-[300px] overflow-y-auto custom-scrollbar">
                                {!user.notes || user.notes.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-40 text-center px-4">
                                        <p className="text-sm font-bold text-slate-400">Sin seguimiento previo</p>
                                        <p className="text-xs text-slate-400/80 mt-1">
                                            No hay registro de contacto para este usuario. Añade una nota para comenzar
                                            el seguimiento.
                                        </p>
                                    </div>
                                ) : (
